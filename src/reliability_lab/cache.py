...
class ResponseCache:
    ...
    def set(self, query: str, value: str, metadata: dict[str, str] | None = None) -> None:
        """Store a response in cache, respecting the privacy guardrail."""
        if _is_uncacheable(query):
            return
    ...
    def similarity(a: str, b: str) -> float:
        ...
        Plain word-overlap (Jaccard) treats "policy" and "policies" as
        unrelated and gives coarse, step-y scores. Mixing in character
        n-grams captures partial/morphological overlap so near-duplicate
        phrasings score high while unrelated phrases score low.
        """
        if a == b:
            return 1.0
        ...
        vec_a, vec_b = vectorize(a), vectorize(b)
...
class SharedRedisCache:
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        ...
        Tries an exact-match hash key first (cheap, O(1)); falls back to a
        SCAN + local similarity comparison for near-duplicate queries, same
        privacy/false-hit guardrails as the in-memory ResponseCache.
        """
        if _is_uncacheable(query):
            return None, 0.0
        ...
        if best_key is not None and best_score >= self.similarity_threshold:
            if _looks_like_false_hit(query, best_query or ""):
                ...
                return None, best_score
            return self._redis.hget(best_key, "response"), best_score

        return None, best_score

    def set(self, query: str, value: str, metadata: dict[str, str] | None = None) -> None:
        """Store a response in Redis with TTL, respecting the privacy guardrail."""
        if _is_uncacheable(query):
            return
