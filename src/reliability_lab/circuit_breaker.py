...
class CircuitBreaker:
    ...
    def allow_request(self) -> bool:
        ...
        CLOSED and HALF_OPEN always allow. OPEN denies until
        reset_timeout_seconds has elapsed since opened_at, at which point we
        flip to HALF_OPEN and allow exactly one probe request through.
        """
    ...
    def call(self, fn: Callable[..., T], *args: object, **kwargs: object) -> T:
        ...
        Fails fast with CircuitOpenError when the breaker denies the request —
        no network/provider call happens at all in that case. Otherwise the
        outcome is recorded so the state machine can react.
        """
    ...
    def record_success(self) -> None:
        """Record a successful call and possibly close the circuit again."""
    ...
    def record_failure(self) -> None:
        ...
        HALF_OPEN-probe-failure and threshold-breach are different triggers
        with different reasons, so they are deliberately separate if/elif
        branches rather than one condition joined with `or`.
        """
        self.failure_count += 1
        self.success_count = 0
        if self.state == CircuitState.HALF_OPEN:
            # A single failed probe re-opens the circuit immediately — no
            # need to accumulate failure_threshold failures again.
