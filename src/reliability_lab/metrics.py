...
class RunMetrics(BaseModel):
    ...
    def write_csv(self, path: str | Path) -> None:
        """Export metrics to a single-row CSV (one column per metric).

        Scenario pass/fail results are flattened into their own
        `scenario_{name}` columns so the whole run fits on one row and is
        easy to diff between runs / paste into a spreadsheet.
        """
